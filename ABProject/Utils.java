package ABProject;

public class Utils
{
    public static final  String  TestDate_Path = "src/test/java/ABProject/resources/TestData.xlsx";

    public static  final  String JsonFile ="src/test/java/ABProject/resources/JsonFile.json";

    public  static  final  String  ExtentReportFile  ="src/test/java/ABProject/resources/ExtentReports.html";

    public static  final  String PropertiesFile = "src/test/java/ABProject/resources/Data.properties";
}

