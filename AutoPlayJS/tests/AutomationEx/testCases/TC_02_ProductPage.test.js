...
import {cart_icon,filter,values,side_Menu_icon,
    All_Items,About,logout,Reset,Card,product_name,product_des,
    product_price,add_to_cart_Button,product_image}from '../pageObjects/productPage';
/*
...
test.describe(`@SecondTestCase : Verify Product Page `,()=>{
    ...
    test('Product Page Functionality and UI checkin ',async({LoginPage,productPage})=>{
        ...
        await test.step ('verify cards  are visible and count match ',async()=>{
            const count  = 6;
    ...
    test('Check the product page functionalitys',async({LoginPage,productPage})=>{
        test.step('Check click add to card ,icon count was increase',async()=>{
            await productPage.check_Add_to_Cart_and_Count();
        })
    })
} )
